...
interface ChatMsg { player_name: string; text: string; }
...
const SLOT_DIST: Record<number, number[]> = { 1:[0], 2:[5,6], 3:[5,0,6], 4:[5,3,4,6], 5:[5,3,0,4,6], 6:[5,3,1,2,4,6], 7:[5,3,1,0,2,4,6] };
...
export default function Spectate() {
  ...
  useEffect(() => {
    ...
    wsRef.current = ws;
    ws.onopen = () => ws.send(JSON.stringify({ type: 'spectate_room', room_id: roomId }));
    ws.onmessage = (e) => {
      ...
      if (msg.type === 'spectate_joined' || msg.type === 'game_update') {
        ...
        setChatMessages(prev => [...prev.slice(-99), { player_name: msg.player_name ?? '?', text: msg.text ?? '' }]);
    ...
    return (
      <div className="lobby">
        <div className="lobby-card">
          ...
          <button className="btn-ghost" style={{ marginTop: 24 }} onClick={() => navigate('/admin')}>← Admin</button>
        </div>
      </div>
    );
  }
    ...
    <div className="app">
      ...
      <header className="hdr">
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: 2, textTransform: 'uppercase', padding: '3px 10px', borderRadius: 5, background: 'rgba(201,168,76,0.12)', color: 'var(--gold)', border: '1px solid rgba(201,168,76,0.28)' }}>
            Spectating
          </span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', color: 'var(--text)' }}>{PHASE_LABEL[phase] ?? phase}</span>
          <span style={{ fontSize: 12, color: 'var(--text)' }}>{player_order.length} player{player_order.length !== 1 ? 's' : ''}</span>
          <button className="btn-ghost leave-btn" onClick={() => navigate('/admin')} title="Back to admin">✕</button>
        </div>
      </header>
          ...
          <div className="table-rim">
            <div className="casino-table">
              ...
              {player_order.map((pid, i) => {
                ...
                const bet = hasCards ? (p.player_hands[0]?.bet ?? p.current_bet) : p.current_bet;
                return (
                  <div key={pid} className={`opp-seat${isActive ? ' opp-seat-active' : ''}`}
                    style={{ position: 'absolute', top: `${slot.top}%`, left: `${slot.left}%`, transform: 'translate(-50%, 0)' }}>
                    ...
                    {hasCards && (
                      <div className="opp-cards">
                        {p.player_hands.map((hand, hi) => (
                          <HandView key={hi} cards={hand.cards} active={isActive && hi === p.active_hand_index} result={phase === 'result' ? (hand.result ?? undefined) : undefined} />
                        ))}
                      </div>
                    )}
                    <div className="opp-bet-row">
                      {p.side_bets.perfect_pairs > 0 && <span className="opp-sb-dot">PP</span>}
                      <div className={`opp-bet-box${bet > 0 ? ' obb-active' : ''}`}>
                        {bet > 0 ? <span>${bet.toLocaleString()}</span> : <span className="obb-empty">BET</span>}
                      </div>
                      {p.side_bets.twenty_one_plus_three > 0 && <span className="opp-sb-dot">21+3</span>}
                    </div>
                  </div>
                );
              })}
      ...
      {chatMessages.length > 0 && (
        <div className="spectate-chat">
          <div className="spectate-chat-msgs">
            {chatMessages.slice(-8).map((m, i) => (
              <div key={i} className="spectate-chat-line">
                <span className="spectate-chat-name">{m.player_name}:</span>
                <span className="spectate-chat-text">{m.text}</span>
              </div>
            ))}
